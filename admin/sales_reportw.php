        ...
        <div class="box-body">
         <!-- general form elements -->
          <div class="box box-primary">
            ...
            <!-- /.box-header -->
            <!-- form start -->
<form action="sales_reportw.php" method="get">
<center><strong>
...
<table class="table table-bordered table-striped" id="table_example" data-responsive="table" >
	...
	<tbody>
        ...
        s.invoice, s.product, s.price, s.quantity, s.amount, d.customer, d.date, d.time, d.entrant, d.wsale
        FROM `medicine_list` AS c
        INNER JOIN `sales_list` AS s ON c.sno = s.sno
        INNER JOIN `sales_details` AS d ON d.id = s.invoice

				WHERE wsale = 'Yes' AND STR_TO_DATE(`date`, '%d/%m/%Y')
	...
	<thead>
		<tr>
			...
			<?php
				...
				$results = $dbo->prepare("SELECT sum(s.amount), d.date, d.wsale FROM sales_list AS s  INNER JOIN `sales_details` AS d ON d.id = s.invoice WHERE wsale = 'Yes' AND STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "'"  );
				$results->execute();
				...
				<th colspan="1" style="border-top:1px solid #999999">
			<?php
				$resultia = $dbo->prepare("SELECT sum(s.profit), d.date, d.wsale FROM sales_list AS s INNER JOIN `sales_details` AS d ON d.id = s.invoice WHERE wsale = 'Yes' AND STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "'"   );
				$resultia->execute();
...
s.invoice, s.product, s.price, s.quantity, s.amount, d.date, d.time, d.entrant, d.wsale
FROM `medicine_list` AS c
INNER JOIN `sales_list` AS s ON c.sno = s.sno
INNER JOIN `sales_details` AS d ON d.id = s.invoice WHERE wsale = 'Yes' AND date BETWEEN '$from' and '$to' ORDER by transaction_id DESC");
