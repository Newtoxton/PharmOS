...
function saveMed( array $data){
		if( !empty( $data ) ){
	      ...
	      $wsale    = mysqli_real_escape_string( $con, trim( $data['wsale'] ) );
				...
				if(empty($id)){
					$uuid = uniqid();
					$query = "INSERT INTO proforma_details (`customer`,`date`, `time`, `entrant`, `wsale`, `cash`, `uuid`)
							VALUES ('$customer',  '$date', '$time', '$userid', '$wsale', '$cash', '$uuid')";

				}else{
					$uuid = $data['uuid'];
					$query = "UPDATE `proforma_details` SET `customer` = '$customer', `amount` = '$totals',`date` ='$date',
    ...
    function saveInvoiceDetail(array $sales_list, $invoice = ''){
	  global $con;
    $deleteQuery = "DELETE FROM proforma_list WHERE invoice = $invoice";
    ...
    foreach ($sales_list as $invoice_detail){
        ...
        $query = "INSERT INTO proforma_list
...
function getpurchases(){
	global $con;
	$data = [];
	$query = "SELECT * FROM proforma_details";
