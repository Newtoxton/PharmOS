...
<div ng-controller="customersCrtl">
    ...
    <div class="row">
		...
		<tr>
            ...
            <a href="reports.xls"><button type="button" class="btn btn-primary pull-right" style="margin-right: 5px;">
                      <i class="fa fa-download"></i> Download Report</button></a>
        </div>
    ...
    $myArr=array("");
    $excel->writeLine($myArr);
    $myArr=array("Brand Name","Generic Name", "Batch No", "Expiry Date", "Quantity Left", "Cost", "Loss");
    $excel->writeLine($myArr);
    $qry=mysqli_query($con, "select c.trade_name, c.generic_name, i.quantity, i.expiry_date , i.batch , i.cost_price, sum(i.quantity * i.cost_price) AS tloss   FROM `medicine_list` AS c INNER JOIN `inventory` AS i ON c.sno = i.sno WHERE date_format(`expiry_date`, '%Y-%m-%d') < curdate()   AND  `quantity` > 0 ORDER BY expiry_date ASC");
    ...
    if($result!=false)
    {
      ...
      {
        $myArr=array($res['trade_name'],$res['generic_name'],$res['batch'],$res['expiry_date'],$res['quantity'],$res['cost_price'],$res['tloss']);
        $excel->writeLine($myArr);
        $i++;
      }
    }
