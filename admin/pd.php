...
?>
                  ...
                  <a href="expiries.xls"><button type="button" class="btn btn-primary pull-right" style="margin-right: 5px;">
...
<div class="table-responsive">
				...
				$result = $dbo->prepare("SELECT c.trade_name, c.generic_name, i.quantity, i.expiry_date , i.batch , i.cost_price, (i.quantity * i.cost_price) AS loss   FROM `medicine_list` AS c INNER JOIN `inventory` AS i ON c.sno = i.sno WHERE date_format(`expiry_date`, '%Y-%m-%d') < NOW() + INTERVAL 3 MONTH  AND  `quantity` > 0 AND date_format(`expiry_date`, '%Y-%m-%d') > curdate() ORDER BY expiry_date ASC");
