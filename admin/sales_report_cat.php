        ...
        <div class="box-body">
         <!-- general form elements -->
          <div class="box box-primary">
            ...
            <!-- /.box-header -->
            <!-- form start -->
<form action="sales_report_cat.php" method="get">
  <div class="container">
		   </br>
              ...
              <div class="row">
               ...
               <div class="col-xs-3">
                  <select name="brand[]"  class="form-group" id="select2" style="width:250px; height:34px" autocomplete="off" multiple>
					<option>Select Category</option>

					<?php

					$result3 = $dbo->prepare("SELECT type FROM medicine_list GROUP BY type ORDER BY type ASC");
...
Sales Report from&nbsp;<?php echo $_GET['d1'] ?>&nbsp;to&nbsp;<?php echo $_GET['d2'] ?>&nbsp;for&nbsp;
...
<table class="table table-bordered table-striped" id="table_example" data-responsive="table" >
	...
	<tbody>
			...
			<?php
				...
				$result = $dbo->prepare("select c.trade_name, c.generic_name,c.type, s.profit,s.transaction_id,
            s.invoice, s.product, s.price, s.quantity, s.amount, d.date, d.time, d.entrant, d.customer
            FROM `medicine_list` AS c
            INNER JOIN `sales_list` AS s ON c.sno = s.sno
            INNER JOIN `sales_details` AS d ON d.id = s.invoice
    				WHERE `type` = '$brand'  AND STR_TO_DATE(`date`, '%d/%m/%Y')
	...
	<thead>
		<tr>
			...
			<?php
				...
				$results = $dbo->prepare("select c.trade_name,c.type, SUM(s.quantity), d.date
        FROM `medicine_list` AS c
        INNER JOIN `sales_list` AS s ON c.sno = s.sno
        INNER JOIN `sales_details` AS d ON d.id = s.invoice

				WHERE `type` = '$brand' AND customer != 'Adjustment'  AND STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' ");
				$results->execute();
			...
			<?php
				...
				$results = $dbo->prepare("
				select c.trade_name, SUM(s.amount), d.date
        FROM `medicine_list` AS c
        INNER JOIN `sales_list` AS s ON c.sno = s.sno
        INNER JOIN `sales_details` AS d ON d.id = s.invoice
				...
				<th colspan="1" style="border-top:1px solid #999999">
			<?php
				$resultia = $dbo->prepare("
				select c.trade_name, SUM(s.profit), d.date
        FROM `medicine_list` AS c
        INNER JOIN `sales_list` AS s ON c.sno = s.sno
        INNER JOIN `sales_details` AS d ON d.id = s.invoice
				WHERE `type` = '$brand' AND customer != 'Adjustment'  AND STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "'"   );
...
$qry=mysqli_query($con, "select c.trade_name, c.generic_name,s.profit,s.transaction_id,
        ...
        INNER JOIN `sales_details` AS d ON d.id = s.invoice
				...
				WHERE `type` = '$brand' AND customer != 'Adjustment' AND STR_TO_DATE(`date`, '%d/%m/%Y')
