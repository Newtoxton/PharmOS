...
function saveMed( array $data){
		if( !empty( $data ) ){
			...
			if( !empty( $data)){
                ...
                $entrant = mysqli_real_escape_string( $con, trim( $data['entrant'] ) );
				...
				if(empty($id)){
					...
					$query = "INSERT INTO purchases (`supplier`, `invoiceNo`, `invoiceDate`,`invoice_total`, `due_date`,
							`amount_paid`, `amount_due`, `notes`, `entrant`, `created`, `uuid`)
							VALUES ('$supplier', '$invoiceNo', '$invoiceDate',  '$invoice_total', '$due_date', '$amount_paid', '$amount_due', '$notes', '$entrant',
