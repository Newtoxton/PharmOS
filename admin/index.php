...
<head>
  ...
  <title>Fabian Pharmacy | Dashboard</title>
  ...
  <div class="content-wrapper">
	...
	$result = $dbo->prepare("SELECT * FROM users ");
				...
				$rowcount = $result->rowcount();
						...
						$query = mysqli_query($con, "SELECT sum(s.amount) AS tsale, d.date FROM sales_list AS s  INNER JOIN `sales_details` AS d ON d.id = s.invoice
					     WHERE STR_TO_DATE(`date`, '%d/%m/%Y')

			 				BETWEEN '" . $start_date . "' AND '" . $end_date . "' AND customer != 'Adjustment' ")or die(mysqli_error());
      ...
      <div class="row">
        <div class="col-lg-3 col-xs-6">
          ...
          <div class="small-box bg-aqua">
            <div class="inner">
              <h3><?php echo $currency  ?> <?php echo formatMoney($row['tsale']); ?></h3>
  <?php } ?>
        ...
        <div class="col-lg-3 col-xs-6">
          ...
          <div class="small-box bg-green">
			...
			<?php $query=mysqli_query($con, "SELECT sum(amount) AS tbill FROM `bills` WHERE STR_TO_DATE(`date`, '%d/%m/%Y')
	...
	<tbody>
			...
			$end_date =  date('Y-12-31');
				...
				$query=mysqli_query($con, "SELECT YEAR(s.datetime) AS `Year`, MONTHNAME(s.datetime) AS `Month`, SUM(s.price * s.quantity) tal FROM `sales_details` AS c INNER JOIN `sales_list` AS s ON c.id = s.invoice WHERE  customer != 'Adjustment' AND STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' GROUP BY YEAR(datetime), MONTH(datetime)")or die(mysqli_error());
			...
			<td><span class="white-text" style="margin-right: 0.1em;"><?php echo $currency  ?></span><?php echo number_format ($row['tal']); ?></td>
          ...
          <div class="box box-success">
			...
			Total Number of Products:  <font color="green" style="font:bold 22px 'Aleo';">[<?php echo $rowcount;?>]</font><br><br>
			</div>
...
$sql = "SELECT
        ...
        SUM(quantity * cost_price)  as 'total_inventory'
        FROM inventory WHERE quantity > 0 AND date_format(`expiry_date`, '%Y-%m-%d') > curdate() ";
...
<script type="application/javascript">
    ...
    user_chart.data = [
    <?php
    $query = mysqli_query($con, "select c.entrant, SUM(s.price * s.quantity) AS Total FROM `sales_details` AS c INNER JOIN `sales_list` AS s ON c.id = s.invoice WHERE customer != 'Adjustment' AND STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' GROUP BY entrant ORDER BY Total DESC LIMIT 5") or die(mysqli_error());
    while ($row = mysqli_fetch_array($query)) {
        ?>
        <?php echo  $row['Total'] . ','; ?>
    <?php }; ?>
