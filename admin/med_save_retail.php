...
function saveMedicine( array $data){
		if( !empty( $data ) ){
			global $dbo;
        return $data;
        ...
        $uuid = uniqid();
				...
				if(empty($id)){
					$query = "INSERT INTO sales_details (`customer`,`date`, `time`, `entrant`, `wsale`, `cash` , `uuid`)
							VALUES (?,?,?,?,?,?,?)";
          ...
          $stmt->bindParam(5, $wsale, PDO::PARAM_INT);
          $stmt->bindParam(6, $cash, PDO::PARAM_INT);
          $stmt->bindParam(7, $uuid, PDO::PARAM_STR);
          return $stmt->execute();
				}else{
					$uuid = $data['uuid'];
					$query = "UPDATE `sales_details` SET `customer` = ?, `amount` = ?,`date` =?,
							`time` = ?, `entrant` = ?,  `updated` = CURRENT_TIMESTAMP
							WHERE `id` = ?";
				}
  ...
  function saveMed( array $data){
		if( !empty( $data ) ){
			...
			if( !empty( $data)){
				...
				if(empty($id)){
					$uuid = uniqid();
					$query = "INSERT INTO sales_details (`customer`,`date`, `time`, `entrant`, `wsale`, `cash` , `uuid`)
							VALUES ('$customer',  '$date', '$time', '$userid', '$wsale', '$cash', '$uuid')";
    ...
    foreach ($sales_list as $invoice_detail){
        ...
        $total = mysqli_real_escape_string( $con, trim( $invoice_detail['total'] ) );
				...
				$qu= mysqli_query($con, "UPDATE inventory SET quantity = '0'  WHERE quantity < 0");
