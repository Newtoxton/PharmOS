...
<table class="table table-bordered table-striped" id="table_example" data-responsive="table" >
	...
	<tbody>
			...
			<?php
			   ...
			   $today =  $_GET['d1'];
				...
				FROM sales_details AS l INNER JOIN sales_list AS t ON l.id = t.invoice WHERE STR_TO_DATE(`date`, '%d/%m/%Y') = '$today' AND customer != 'Adjustment' GROUP BY invoice 
	...
	<tr>
				...
				$results = $dbo->prepare("SELECT sum(s.quantity * s.price) , d.date FROM sales_list AS s  INNER JOIN `sales_details` AS d ON d.id = s.invoice  WHERE STR_TO_DATE(`date`, '%d/%m/%Y') = '$today' AND customer != 'Adjustment' ");
				$results->execute();
