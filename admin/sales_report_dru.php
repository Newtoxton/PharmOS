        ...
        <div class="box-body">
         <!-- general form elements -->
          <div class="box box-primary">
            ...
            <!-- /.box-header -->
            <!-- form start -->
<form action="sales_report_dru.php" method="get">
  <div class="container">
		   </br>
...
<table class="table table-bordered table-striped" id="table_example" data-responsive="table" >
	...
	<tbody>
            ...
            INNER JOIN `sales_details` AS d ON d.id = s.invoice
    				...
    				WHERE `trade_name` = '$brand'  AND STR_TO_DATE(`date`, '%d/%m/%Y')
	...
	<thead>
		<tr>
			...
			<?php
				...
				$results = $dbo->prepare("select c.trade_name, SUM(s.quantity), d.date
        FROM `medicine_list` AS c
        INNER JOIN `sales_list` AS s ON c.sno = s.sno
        INNER JOIN `sales_details` AS d ON d.id = s.invoice

				WHERE `trade_name` = '$brand'  AND STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' ");
				$results->execute();
        ...
        INNER JOIN `sales_list` AS s ON c.sno = s.sno
        INNER JOIN `sales_details` AS d ON d.id = s.invoice
				WHERE `trade_name` = '$brand'  AND customer != 'Adjustment' AND STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "'"   );
				$resultia->execute();
...
$qry=mysqli_query($con, "select c.trade_name, c.generic_name,s.profit,s.transaction_id,
        ...
        INNER JOIN `sales_details` AS d ON d.id = s.invoice
				...
				WHERE `trade_name` = '$brand'  AND customer != 'Adjustment'  AND STR_TO_DATE(`date`, '%d/%m/%Y')
