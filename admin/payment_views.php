...
<table class="table table-bordered table-striped" id="table_example" data-responsive="table" >
	...
	<tbody>
            ...
            $daten =    $_GET['date'] ;
				...
				$result = $dbo->prepare("SELECT p.supplier,p.id,  p.invoiceDate, p.invoiceNo, (m.id) mid, m.paid,m.ck_no, DATE(m.datetime) tdate FROM purchases AS p INNER JOIN
				supplier_pay AS m ON p.id = m.t_id
				WHERE `supplier` = '$supplier'  AND DATE(m.datetime) = '$daten'  ORDER by t_id DESC");
      ...
      <td><form  method="post" action="delete_payments.php?id=<?php echo $mid; ?>"><input type='submit'  class='btn btn-danger delete' value='Delete'>	</form></td>
	...
	<thead>
		<tr>
			...
			<?php
				...
				supplier_pay AS m ON p.id = m.t_id
				WHERE `supplier` = '$supplier' AND DATE(m.datetime) = '$daten' ");
				$results->execute();
