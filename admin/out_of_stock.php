...
<a href="reports.xls"><button type="button" class="btn btn-primary pull-right" style="margin-right: 5px;">
        ...
        $myArr=array("");
        $excel->writeLine($myArr);
        $myArr=array("Trade Name","Quantity");
        $excel->writeLine($myArr);
        $qry=mysqli_query($con, "select c.trade_name, SUM(GREATEST(i.quantity,0)) AS Qty FROM `medicine_list` AS c INNER JOIN `inventory` AS i ON c.sno = i.sno  GROUP BY i.sno HAVING QTY <= 0");
        ...
        if($result!=false)
        {
        	...
        	{
        		$myArr=array($res['trade_name'],$res['Qty']);
        		$excel->writeLine($myArr);
        		$i++;
        	}
        }
