...
WHERE supplier != 'Adjustment' AND supplier != 'STOCK TAKING'
GROUP BY supplier ORDER BY supplier ASC")or die(mysqli_error());
            ...
            <td><form  method="post" action="debt_aging.php?supplier=<?php echo $row['supplier']; ?>"><input type='submit'  ' class="btn btn-success addmore" value='View'>	</form></td>
		...
		<tr>
			...
			<?php
				 $query=mysqli_query($con, "SELECT SUM(i.cost_price *  i.qty_sold) total  FROM purchases AS p INNER JOIN
                    inventory AS i ON p.id = i.invoice_id WHERE supplier != 'Adjustment' AND supplier != 'STOCK TAKING'")or die(mysqli_error());
						while($row=mysqli_fetch_array($query)){
							$total = $row['total'];
			...
			<?php
				 $query=mysqli_query($con, "SELECT p.supplier, SUM(s.paid) paid FROM purchases AS p
                                            LEFT JOIN supplier_pay AS s ON p.id = s.t_id WHERE supplier != 'Adjustment' AND supplier != 'STOCK TAKING'
					")or die(mysqli_error());
						while($row=mysqli_fetch_array($query)){
