...
INNER JOIN `purchases` p ON p.id = i.invoice_id


WHERE STR_TO_DATE(`invoiceDate`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' AND  invoice_id != '2' GROUP BY type ORDER BY type ASC");
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
			...
			<?php
	$result = $dbo->prepare("SELECT p.invoiceDate, p.supplier, SUM(i.cost_price *  i.qty_sold) paid FROM purchases AS p INNER JOIN inventory AS i
	ON p.id = i.invoice_id 


WHERE STR_TO_DATE(`invoiceDate`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' AND  invoice_id != '2' ");
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
...
<input style="border:none" type="text" id="gprofit" onchange="updateDue()" />
 ...
 <thead>
		...
		<th> Expenses </th>
							...
							<?php
	$result = $dbo->prepare("SELECT name, SUM(amount)AS expense FROM bills WHERE STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "'  AND name !='Assets' AND name !='Loan' GROUP BY name ORDER BY name  ASC");
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
			...
			<?php
	$result = $dbo->prepare("Select  SUM(amount) AS t_bill FROM bills WHERE STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' AND name !='Assets' AND name !='Loan'");
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
