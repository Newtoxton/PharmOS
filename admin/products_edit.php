...
<table class="table table-bordered table-striped" id="table_example" data-responsive="table" >
	...
	<tbody>
			...
			<?php
				...
				$result = $dbo->prepare("select c.trade_name, c.generic_name, i.id, i.batch, i.cost_price, i.sell_price, i.supplier, i.quantity, i.qty_sold, date_format(i.datetime, '%d/%m/%y') AS DATE, i.expiry_date FROM `medicine_list` AS c INNER JOIN `inventory` AS i ON c.sno = i.sno  ORDER BY i.id Desc");
