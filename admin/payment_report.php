...
Payment Report from&nbsp;<?php echo $_GET['d1'] ?>&nbsp;to&nbsp;<?php echo $_GET['d2'] ?>&nbsp;for&nbsp;<?php echo $_GET['customer'] ?>
</div>
...
<table class="table table-bordered table-striped" id="table_example" data-responsive="table" >
	...
	<tbody>
			...
			<?php
				...
				$result = $dbo->prepare("SELECT l.id, l.customer, l.date,(m.id) mid, SUM(m.paid) paid, DATE(m.datetime) tdate, m.hand FROM sales_details AS l INNER JOIN
				credit_pay AS m ON l.id = m.t_id
				WHERE `customer` = '$customer'  AND DATE(m.datetime)
                BETWEEN '" . $start_date . "' AND '" . $end_date . "'
                GROUP BY tdate, customer ORDER by t_id DESC");
          ...
          <td><?php echo $row['hand']; ?></td>
			...
			<td><form  method="post" action="payment_view.php<?php echo '?customer='.$row['customer']; ?>&<?php echo 'date='.$row['tdate']; ?>"><input type='submit'  class="btn btn-success addmore" value='View'>	</form></td>
	...
	<thead>
		<tr>
			...
			<?php
				...
				credit_pay AS m ON l.id = m.t_id
				WHERE `customer` = '$customer' AND DATE(m.datetime) BETWEEN '" . $start_date . "' AND '" . $end_date . "' ");
				$results->execute();
