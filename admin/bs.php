...
<div class="box">
 ...
 <h3><center>Period From&nbsp;<?php echo $_GET['d1'] ?>&nbsp;To&nbsp;<?php echo $_GET['d2'] ?></h3></center>
	...
	$result = $dbo->prepare("SELECT sum(s.price * s.quantity) AS tsale, d.date FROM sales_list AS s INNER JOIN `sales_details` AS d ON d.id = s.invoice
  WHERE customer != 'Adjustment' AND cash = 'Yes' AND STR_TO_DATE(`date`, '%d/%m/%Y')
...
?>
			 ...
			 <?php
	   $query=mysqli_query($con, "SELECT sum(s.price * s.quantity) AS amount, d.date FROM sales_list AS s INNER JOIN `sales_details` AS d ON d.id = s.invoice
     WHERE customer != 'Adjustment' AND cash = 'No'  AND STR_TO_DATE(`date`, '%d/%m/%Y')
	   ...
	   FROM credit_pay AS m LEFT JOIN sales_details AS l ON l.id = m.t_id  WHERE customer != 'Adjustment' AND cash ='No' AND STR_TO_DATE(`date`, '%d/%m/%Y')
       BETWEEN '" . $start_date . "' AND '" . $end_date . "' ")or die(mysqli_error());
	   while($row=mysqli_fetch_array($query)){
...
WHERE supplier != 'Adjustment' AND STR_TO_DATE(`invoiceDate`, '%m/%d/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' ")or die(mysqli_error());
						while($row=mysqli_fetch_array($query)){
...
INNER JOIN `supplier_pay` AS s ON p.id = s.t_id WHERE supplier != 'Adjustment' AND STR_TO_DATE(`invoiceDate`, '%m/%d/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "'")or die(mysqli_error());
						while($row=mysqli_fetch_array($query)){
