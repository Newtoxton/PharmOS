...
<table class="table table-bordered table-striped" id="table_example" data-responsive="table" >
	...
	<tbody>
        ...
        INNER JOIN `sales_details` AS d ON d.id = s.invoice
				...
				WHERE  customer != 'Adjustment' AND STR_TO_DATE(`date`, '%d/%m/%Y')
	...
	<thead>
		<tr>
			...
			<?php
				...
				$results = $dbo->prepare("SELECT sum(s.amount), d.date FROM sales_list AS s  INNER JOIN `sales_details` AS d ON d.id = s.invoice  WHERE customer != 'Adjustment' AND STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' "  );
				$results->execute();
