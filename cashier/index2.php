  ...
  <div class="content-wrapper">
	...
	$result = $dbo->prepare("SELECT * FROM users ");
				...
				$rowcount = $result->rowcount();
						...
						$query = mysqli_query($con, "SELECT sum(amount) AS tsale FROM `sales_list` WHERE  DATE(datetime) = CURDATE();")or die(mysqli_error());
  ...
  <?php } ?>
        ...
        <div class="col-lg-3 col-xs-6">
          ...
          <div class="small-box bg-green">
			...
			<?php $query=mysqli_query($con, "SELECT sum(amount) AS tbill FROM `bills` WHERE  DATE(datetime) = CURDATE();")or die(mysqli_error());
						while($row=mysqli_fetch_array($query)){
						?>
...
$level = $row['level'];
	...
	?>
			<div style="text-align:center;">
			 Products are below re-order level of <?php echo $level;?> : <font style="color:rgb(255, 95, 66);; font:bold 22px 'Aleo';">[<?php echo $rowcount123;?>]</font><br><br>
			</div>
...
<script type="application/javascript">
    ...
    user_chart.data = [
    <?php
    $query = mysqli_query($con, "select c.entrant, SUM(s.amount) AS Total FROM `sales_details` AS c INNER JOIN `sales_list` AS s ON c.id = s.invoice WHERE datetime BETWEEN '" . $start_date . "' AND '" . $end_date . "' GROUP BY entrant ORDER BY Total DESC LIMIT 5") or die(mysqli_error());
    while ($row = mysqli_fetch_array($query)) {
        ?>
        <?php echo  $row['Total'] . ','; ?>
    <?php }; ?>
