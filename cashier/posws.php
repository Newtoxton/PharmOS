...
if( !empty( $_POST ) ){
	try{
		...
		if( isset($data['success']) && $data['success']){
			...
			header('Location: salesw.php');exit;
		} else {
...
<html lang="en">
  ...
  <link rel="stylesheet" href="../dist/css/skins/_all-skins.min.css">
    ...
    var x = document.forms["form1"]["customer"].value;
		  ...
		  <div class="box">
            <div class="box-header">
			             ...
			             <!-- /.box-header -->
			 						...
			 						<option></option>
			 						<?php
			 						$result = $dbo->prepare("select c.trade_name, c.generic_name,c.wsell2,  i.pid, i.cost_price, i.quantity, i.qty_sold, i.expiry_date FROM `medicine_list` AS c INNER JOIN `inventory` AS i ON c.sno = i.sno  WHERE quantity > 0;");
			 						$result->bindParam(':userid', $res);
			 						$result->execute();
			 						for($i=0; $row = $result->fetch(); $i++){
			 						?>
			 						<option value="<?php echo $row['pid'];?>"> <?php echo $row['trade_name']; ?>  | <?php echo $row['generic_name']; ?> | Cost @ <?php echo $row['cost_price']; ?> | Price @ <?php echo $row['wsell2']; ?> | Expires on: <?php echo $row['expiry_date']; ?> | Qty Left: <?php echo $row['quantity']; ?></option>
			 						<?php
			 						 }
			 						?>
			 						</select>
			...
			<div class="row">
				<div class="col-xs-6">
					...
					?>
												...
												<input type="hidden" name="cash" value="Yes" />
