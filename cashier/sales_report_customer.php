        ...
        <div class="box-body">
         <!-- general form elements -->
          <div class="box box-primary">
            ...
            <!-- /.box-header -->
            <!-- form start -->
<form action="sales_report_customer.php" method="get">
  <div class="container">
		   </br>
...
<table class="table table-bordered table-striped" id="table_example" data-responsive="table" >
	...
	<tbody>
			...
			<?php
				...
				$result = $dbo->prepare("select  SUM(s.profit) profit, s.invoice, SUM(s.amount) amount, d.date, d.time, d.entrant, d.customer
				FROM `medicine_list` AS c
				INNER JOIN `sales_list` AS s ON c.sno = s.sno
				INNER JOIN `sales_details` AS d ON d.id = s.invoice


				WHERE `customer` = '$name'  AND  STR_TO_DATE(`date`, '%d/%m/%Y')
	...
	<thead>
		<tr>
			...
			<?php
				...
				$results = $dbo->prepare("SELECT sum(s.amount), d.date FROM sales_list AS s  INNER JOIN `sales_details` AS d ON d.id = s.invoice  WHERE `customer` = '$name'  AND  STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' "  );
				$results->execute();
				...
				<th colspan="1" style="border-top:1px solid #999999">
			<?php
				$resultia = $dbo->prepare("SELECT sum(s.profit), d.date FROM sales_list AS s INNER JOIN `sales_details` AS d ON d.id = s.invoice WHERE `customer` = '$name' AND  STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "'"   );
				$resultia->execute();
