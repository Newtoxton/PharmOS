        ...
        <div class="box-body">
         <!-- general form elements -->
          <div class="box box-primary">
			...
			<?php
	$id=$_GET['id'];
	$result = $dbo->prepare("Select  s.id, s.name, sum( p.total_bill) AS bill, sum(p.amount_paid) AS paid, sum(p.total_bill)-sum(p.amount_paid) AS Balance FROM `supplier` AS s INNER JOIN `supplier_payment` AS p  ON s.id = p.id WHERE s.id= :userid GROUP BY s.id ");
           ...
           <form role="form" method="post"  action="updatepay.php" >
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <input type="text"  name="address" class="form-control" id="skills"  value="<?php echo $row['bill']; ?>" readonly>
				 <div class="form-group">
                  <label for="exampleInputEmail1">Amount Paid</label>
                  <input type="text"  name="phone"  class="form-control" id="exampleInputEmail1"  value="<?php echo $row['paid']; ?>" readonly>
                </div>
                <div class="form-group">
				<label for="exampleInputEmail1">Amount Due</label>
                  <input type="text"  name="contact_person" class="form-control" id="skills"  value="<?php echo $row['Balance']; ?>" readonly>
                </div>
