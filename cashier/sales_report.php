...
<div class="container">
              ...
              <div class="col-xs-3">
				...
				<div class="col-xs-2">
		<input type="text"  id="from"  name="d1" placeholder="From"  style="height:34px"/> 
                </div>
				
		<div class="col-xs-2">
		 <input type="text" id="to" name="d2" placeholder="To"  style="height:34px"/>
                </div>
...
FROM `medicine_list` AS c INNER JOIN `sales_list` AS s ON c.sno = s.sno INNER JOIN `sales_details` AS d ON d.id = s.invoice WHERE `wsale` = '$type'  AND  STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' ORDER by transaction_id DESC");
