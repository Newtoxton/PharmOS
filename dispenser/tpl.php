...
?>
 ...
 <thead>
		...
		<th> Cost of Goods </th>
							...
							<?php
	$result = $dbo->prepare("SELECT supplier, sum(invoice_total) AS bill, sum(amount_paid) AS paid, sum(amount_due) AS Balance FROM purchases 
	WHERE STR_TO_DATE(`invoiceDate`, '%m/%d/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' GROUP BY supplier ORDER BY paid DESC");
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
			...
			<?php
	$result = $dbo->prepare("Select  sum(invoice_total) AS paid FROM `purchases` WHERE STR_TO_DATE(`invoiceDate`, '%m/%d/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' ");
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
...
<input style="border:none" type="text" id="gprofit" onchange="updateDue()" />
 ...
 <thead>
		...
		<th> Expenses </th>
							...
							<?php
	$result = $dbo->prepare("SELECT name, SUM(amount)AS expense FROM bills WHERE STR_TO_DATE(`date`, '%m/%d/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' GROUP BY name ORDER BY expense DESC");
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
			...
			<?php
	$result = $dbo->prepare("Select  SUM(amount) AS t_bill FROM bills WHERE STR_TO_DATE(`date`, '%m/%d/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "'");
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
