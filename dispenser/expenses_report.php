...
$myArr=array("");
$excel->writeLine($myArr);
$myArr=array("Date","Entrant","Payment type","Description","Total bill","Amount paid");
$excel->writeLine($myArr);
$qry=mysqli_query($con, "select* FROM bills WHERE STR_TO_DATE(`date`, '%m/%d/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' ORDER by id DESC");
...
if($result!=false)
{
	...
	{
		$myArr=array($res['date'],$res['entrant'],$res['type'],$res['description'],$res['total_amount'],$res['amount']);
		$excel->writeLine($myArr);
		$i++;
	}
}
