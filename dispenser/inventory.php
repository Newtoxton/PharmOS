                  ...
                  <th style="text-align:center;">View</th>
				    ...
				    <th style="text-align:center;">Add</th>
                            ...
                            <tbody>
								<?php
                $n = 0;
								$result= mysqli_query($con, "SELECT p.id,p.supplier, p.invoiceNo,p.notes, p.due_date, p.invoiceDate, SUM(p.amount_paid) paid, SUM(i.cost_price *  i.qty_sold) total FROM purchases as p INNER JOIN inventory as i ON 
p.id = i.invoice_id  GROUP BY invoice_id ORDER BY pid DESC" ) or die (mysql_error());
...
<td><form  method="post" action="add_stocks.php<?php echo '?invoice_id='.$row['id']; ?>"><input type='submit'  class="btn btn-success addmore" value='Add'>	</form></td>
