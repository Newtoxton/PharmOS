...
<table class="table table-bordered table-striped" id="table_example" data-responsive="table" >
	...
	<tbody>
        ...
        INNER JOIN `sales_details` AS d ON d.id = s.invoice
				...
				WHERE d.entrant = '$user' AND STR_TO_DATE(`date`, '%d/%m/%Y')
	...
	<thead>
		<tr>
			...
			<?php
				...
				$results = $dbo->prepare("SELECT sum(s.amount), d.date FROM sales_list AS s  INNER JOIN `sales_details` AS d ON d.id = s.invoice  WHERE d.entrant = '$user' AND STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' "  );
				$results->execute();
...
$myArr=array("");
$excel->writeLine($myArr);
$myArr=array("Date","Time","Trade Name","Generic Name" ,"Quantity","Amount","Dispenser");
$excel->writeLine($myArr);
$qry=mysqli_query($con, "select c.trade_name, c.generic_name,s.profit,s.transaction_id, s.invoice, s.product, s.price, s.quantity, s.amount, d.date, d.time, d.entrant
FROM `medicine_list` AS c INNER JOIN `sales_list` AS s ON c.sno = s.sno INNER JOIN `sales_details` AS d ON d.id = s.invoice WHERE d.entrant = '$user' AND STR_TO_DATE(`date`, '%m/%d/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' ORDER by transaction_id DESC");
...
if($result!=false)
{
	...
	{
		$myArr=array($res['date'],$res['time'],$res['trade_name'],$res['generic_name'],$res['quantity'],$res['amount'],$res['entrant']);
		$excel->writeLine($myArr);
		$i++;
	}
}
