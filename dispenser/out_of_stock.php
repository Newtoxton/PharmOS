...
<table class="table table-bordered table-striped" id="table_example" data-responsive="table" >
	...
	<tbody>
			...
			<?php
				...
				$result = $dbo->prepare("select c.trade_name, SUM(GREATEST(i.quantity,0)) AS Qty FROM `medicine_list` AS c INNER JOIN `inventory` AS i ON c.sno = i.sno  GROUP BY i.sno HAVING sum(i.quantity) <= 0");
